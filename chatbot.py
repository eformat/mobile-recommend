...
MODEL_NAME = os.getenv("MODEL_NAME", "Llama-3.2-1B-Instruct-Q8_0.gguf")
...
DB_COLLECTION_NAME = os.getenv("DB_COLLECTION_NAME", "mobile_phones")
...
if re.search(r"LLama-3", MODEL_NAME, flags=re.IGNORECASE):
    ...
    You are a helpful, respectful and honest assistant answering questions about Mobile Phones.
    Only answer the question based on the corpus, price from the context.
    ...
    If a question does not make any sense, or cannot be answered from the context, or is not factually coherent, explain why instead of answering something not correct.
...
def remove_source_duplicates(input_list):
    ...
    for item in input_list:
        if item.metadata['name'] not in unique_list:
            unique_list.append(
                cl.Text(name=item.metadata['name'], content=item.metadata['imgURL'], display="inline")
            )
            unique_list.append(
                cl.Image(name=item.metadata['name'], url=item.metadata['imgURL'], display="inline")
            )
